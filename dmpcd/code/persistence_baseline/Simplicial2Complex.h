...
public:
    ...
    void retrieve_1manifold_simp(string maxFile);
	...
	void PhatPersistence(string persistenceFile);
...
//TODO Lucas
void Simplicial2Complex::retrieve_1manifold_simp(string maxFile){
    output_manifold_vert.clear();
...
void Simplicial2Complex::convert_output(){
    ...
    for(auto it=output_manifold_vert.begin();it!=output_manifold_vert.end();it++){
	    ...
	    int out_vind[2] = {one_mani[i],one_mani[i+1]};
	    /*
            for(int j=0;j<2;j++){
...
void Simplicial2Complex::buildComplexFromFile2_BIN(string pathname) {
	...
	for (int i = 0; i < numOfVertices; i++) {
		...
		if (coords[4019] != 0)
		{
			cout << "coord check: " << coords[4019] << endl;
		}
		*/
	...
	for (int i = 0; i < numOfEdges; i++) {
		...
		//cout << "edge " << i << ": " << vIndex1 << ", " << vIndex2 << endl; 
...
void Simplicial2Complex::PhatPersistence(string persistenceFile){
...
*/
	...
	ofstream pFile(persistenceFile);
	...
	for( phat::index idx = 0; idx < pairs.get_num_pairs(); idx++ ){
		...
		if (s1->dim == 0){
			...
			pFile << 0 << " " << v->funcValue << " " << e->funcValue << endl;
			// use SORTED position
			...
			e->critical_type = 2;
			e->persistence = persistence;
			pFile << 1 << " " << e->funcValue << " " << t->funcValue << endl;
