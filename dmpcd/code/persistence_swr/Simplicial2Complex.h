...
public:
    ...
    void retrieve_1manifold_simp(string vpath_file);

    void build_spt3(float sigma, string efile);
	...
	int findEdge(int v1, int v2){
		//cout << "finding edge for " << v1 << " " << v2 << endl;
	...
	void buildComplexFromFile_RIPS(string pathname);
    void buildComplexAndFiltrationFromFile_RIPS(string pathname);
    ...
    void buildFiltrationWithLowerStar_RIPS();
...
void Simplicial2Complex::build_spt3(float sigma, string efile){
	ofstream edgefile(efile);
...
void Simplicial2Complex::retrieve_1manifold_simp(string vpath_file){
    ofstream vpfile(vpath_file);
	output_manifold_vert.clear();
...
cri_e_vec.insert(cri_e_vec.end(), unpaired.begin(), unpaired.end());
    ...
    cout<<"count cri_e: "<<cri_e_vec.size()<<endl;
    int n_know_min=0;
    int n_not_know=0;
    for(auto it=cri_e_vec.begin();it!=cri_e_vec.end();it++){
        int e_ind = *it;
	    ...
	    for (int index = 0; index < vPath.size() - 1; index++)
	    {
		vpfile << vPath[index] << ' ';
	    }
	    vpfile << vPath[vPath.size() - 1] << "\n";
            output_manifold_vert.push_back(vPath);
	    //cout << "mani size: " << vPath.size() << endl;
        }
    }
    cout<<"#know_min:"<<n_know_min<<endl;
    cout<<"#not know min:"<<n_not_know<<endl;
...
void Simplicial2Complex::convert_output(){
    ...
    for(auto it=output_manifold_vert.begin();it!=output_manifold_vert.end();it++){
        ...
        for(int i=0;i<one_mani.size()-1;i++){
            ...
            int out_vind[2] = {one_mani[i],one_mani[i+1]};//for edge
	    /*
            for(int j=0;j<2;j++){
    ...
    for(auto it=ve_e_gt_sigma.begin();it!=ve_e_gt_sigma.end();it++){
        ...
        int *e_vert = cri_e->getVertices();
        //2ADJ: e_vert[0] e_vert[1]
        int out_vind[2] = {e_vert[0],e_vert[1]};//for edge
	/*
        for(int j=0;j<2;j++){
...
//    }
    ...
    for(auto it=unpaired.begin();it!=unpaired.end();it++){
        ...
        vector<int> new_e{out_vind[0],out_vind[1],3};
        output_edge.push_back(new_e);
    }
...
void Simplicial2Complex::buildComplexAndFiltrationFromFile_RIPS(string pathname)
{
    string line;
    ifstream input_file(pathname);
    //int counter = 0;
    int vert_counter = 0;
    int edge_counter = 0;
    int tri_counter = 0;
    std::vector<int> filtration_dim_tracker;
    filtration_dim_tracker.clear();
    //cout << "starting read in" << endl;
    while ( getline (input_file,line) )
    {
        int total = vert_counter + edge_counter + tri_counter;
        if (total % 10000 == 0)
        {
            cout << "read in " << total << endl;
        }
        /*
        if (counter > 0)
        {
            //cout << "0 ptr check: " << atV(0) << endl;
            //cout << "0 dim check: " << atV(0)->dim << endl;
            cout << "0 dim check filtration: " << filtration[0]->dim << endl;
        }
        */
      string line_no_type = line.substr(2);
      int type = stoi(line.substr(0,1));
      //cout << "line: " << line << endl;
      if (type == 0)
      {
        //cout << "vert: " << vert_counter << endl;
        //cout << line_no_type << endl;
        double coords[1];
        double funcValue;
        int space = line_no_type.find(" ");
        coords[0] = stod(line_no_type.substr(0, space));
        funcValue = stod(line_no_type.substr(space + 1));
        //cout << coords[0] << ' ' << funcValue << endl;
        Vertex v(coords, funcValue);
        addVertex(v);
        ...
        filtration_dim_tracker.push_back(0);

        vert_counter++;
      }
      else if (type == 1)
      {
        //cout << "edge: " << edge_counter << endl;
        int vIndex1, vIndex2;
        double funcValue;
        int first_space = line_no_type.find(" ");
        vIndex1 = stoi(line_no_type.substr(0, first_space));
        string line_second_v = line_no_type.substr(first_space + 1);
        int second_space = line_second_v.find(" ");
        vIndex2 = stoi(line_second_v.substr(0, second_space));
        funcValue = stod(line_second_v.substr(second_space + 1));
        ...
        Edge* added_e = atE(edge_counter);
        added_e->setFuncValue(funcValue);

        vector<int>* adj_e = new vector<int>;
        adj_e->reserve(8);
        e2t.push_back(adj_e);

        filtration_dim_tracker.push_back(1);

        edge_counter++;
      }
      else //type == 2
      {
       // cout << "tri: " << tri_counter << endl;
        int vIndex1, vIndex2, vIndex3;
        double funcValue;
        int first_space = line_no_type.find(" ");
        vIndex1 = stoi(line_no_type.substr(0, first_space));
        ...
        int second_space = line_second_v.find(" ");
        vIndex2 = stoi(line_second_v.substr(0, second_space));
        string line_third_v = line_second_v.substr(second_space + 1);
        int third_space = line_third_v.find(" ");
        vIndex3 = stoi(line_third_v.substr(0, third_space));
        funcValue = stod(line_third_v.substr(third_space + 1));
        ...
        int t = addTriangle(t_vert);
        Triangle* added_t = atT(tri_counter);
        added_t->setFuncValue(funcValue);

        filtration_dim_tracker.push_back(2);
    ...
    */
    ...
    cout << "building filtration" << endl;
    int vert_index = 0;
    int edge_index = 0;
    int tri_index = 0;
    int counter = 0;

    for (int i = 0; i < filtration_dim_tracker.size(); i++)
    {
        int ith_dim = filtration_dim_tracker[i];
        if (ith_dim == 0)
        {
            Vertex* v = atV(vert_index);
            filtration.push_back(v);
            (v)->filtrationPosition = counter++;
            vert_index++;
        }
        else if (ith_dim == 1)
        {
            Edge* e = atE(edge_index);
            filtration.push_back(e);
            (e)->filtrationPosition = counter++;
            edge_index++;
        }
        else //ith dim == 2
        {
            Triangle* t = atT(tri_index);
            filtration.push_back(t);
            (t)->filtrationPosition = counter++;
            tri_index++;
        }
    }

    input_file.close();
    cout << filtration[0]->dim << " should be zero" << endl;
}

void Simplicial2Complex::buildComplexFromFile_RIPS(string pathname)
{
    // Input filename
    ifstream file(pathname, ios::binary);
    ...
    for (int i = 0; i < numOfVertices; i++) {
        ...
        funcValue = *double_reader;
        // funcValue = (int)(funcValue*1e5)/1.0e5;
        //cout << "func value test: " << funcValue << endl;
        Vertex v(coords, funcValue);
        addVertex(v);       // all related processing moved here.
    ...
    */
    ...
    for (int i = 0; i < numOfEdges; i++) {
        int vIndex1, vIndex2;
        double vIndex1_d, vIndex2_d;
        double funcValue;
        file.read(double_buffer, sizeof(double));
        vIndex1_d = *double_reader;
        file.read(double_buffer, sizeof(double));
        vIndex2_d = *double_reader;

        vIndex1 = (int) vIndex1_d;
        vIndex2 = (int) vIndex2_d;

        file.read(double_buffer, sizeof(double));
        funcValue = *double_reader;

        //cout << "edge " << i << ": " << vIndex1 << ", " << vIndex2 << ", " << funcValue << endl;
...
//    }
	...
	if (i % 100000 == 0)
	{
		cout << "working on tri " << i << endl;
	}
        double vIndex1_d, vIndex2_d, vIndex3_d;
        int vIndex1, vIndex2, vIndex3;
        double funcValue;
        file.read(double_buffer, sizeof(double));
        vIndex1_d = *double_reader;
        ...
        vIndex2_d = *double_reader;
        file.read(double_buffer, sizeof(double));
        vIndex3_d = *double_reader;

        vIndex1 = (int) vIndex1_d;
        vIndex2 = (int) vIndex2_d;
        vIndex3 = (int) vIndex3_d;

	//cout << i << ": " << vIndex1 << " " << vIndex2 << " " << vIndex3 << " ";
	...
	//cout << "t_vert finished" << endl;
        //cout << "tri " << i << ": " << vIndex1 << ", " << vIndex2 << ", " << vIndex3 << ", " << funcValue << endl;
        
        int t = addTriangle(t_vert);
	//cout << "triangle added" << endl;
        // triangles in e2t are inserted.
        Triangle* added_t = atT(i);
	//cout << "at T" << endl;
        added_t->setFuncValue(funcValue);
	//cout << "function value set" << endl;
    }
    for (int i = 0; i < numOfTris; i++) {
        addCriticalPoint((Simplex*) atT(i));
    }
...
void Simplicial2Complex::buildFiltrationWithLowerStar_RIPS() {
/*Reserve space, so filtration doesn't need to resize as often*/
    filtration.reserve(order());
    unordered_set<Simplex*> dup_test;
    dup_test.clear();
    ...
    */
    ...
    cout << "inserting edges and triangles..." << endl;
    map<int, int> tri_edge_count;
    ...
    {
	cout << "working on edge " << i << " out of " << edgeList.size()  << endl;
        Edge* e = atE(i);
	double e_func_value = e->funcValue;
	for (int j = vert_index; j < vertexList.size(); j++)
	{
		Vertex* v = atV(j);
		double v_func_value = v->funcValue;
		if (v_func_value > e_func_value)
		{
			break;
		}
		//cout << "Adding vert " << j << endl;
		filtration.push_back(v);
		(v) ->filtrationPosition = counter ++;
		vert_index++;
	}
        ...
        int v0 = vertices[0];
        int v1 = vertices[1];
        //cout << "adding edge " << v0 << " " << v1 << endl;
        //cout << filtration.size() << endl;
        for (int j = tri_index; j < triList.size(); j++)
        {
            Triangle* t = atT(j);
            int* t_vertices = t->getVertices();
            int t0 = t_vertices[0];
            int t1 = t_vertices[1];
            int t2 = t_vertices[2];
            if ((v0 == t0 && v1 == t1) || (v0 == t1 && v1 == t0) || (v0 == t0 && v1 == t2) || 
                (v0 == t2 && v1 == t0) || (v0 == t1 && v1 == t2) || (v0 == t2 && v1 == t1))
            {
                tri_edge_count[j]++;
                if (tri_edge_count[j] == 3)
                {
                    //cout << "adding tri " << t0 << " " << t1 << " " << t2 << endl;
                    filtration.push_back(t);
                    (t) ->filtrationPosition = counter ++;
                    tri_index++;
                    //cout << filtration.size() << endl;
...
void Simplicial2Complex::PhatPersistence(){
	...
	for (unsigned int i = 0; i < this->filtration.size(); i++){
        //cout << "building boundary for " << i << endl;
		Simplex *s = filtration[i];
        //cout << "got simplex " << s->dim << endl;
		temp_col.clear();
	...
	double elapsed_secs_ = double(end - begin) / CLOCKS_PER_SEC;
	cout << "!!!!!!!!!actual persistence: " << elapsed_secs_ <<endl;
	// pairs.sort();
