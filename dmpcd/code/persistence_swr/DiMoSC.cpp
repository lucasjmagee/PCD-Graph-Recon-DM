...
#define MAX_DIM 4020			// - Will be used in Simplex.h
...
int main(int argc, char* argv[]){
	...
	edge_info_file = string(argv[2]) + "/edge_for_morse_only.txt";
	vpath_file = string(argv[2]) + "/vpath.txt";
	...
	if (!use_pre_save){
		...
		K.buildComplexAndFiltrationFromFile_RIPS(argv[1]);
		cout << "Done\n";
		cout.flush();
		// cin.get(); // has test
        ...
        cout<<"read in and build complex and filtration: "<<elapsed_secs_<<endl;
...
// build tree
    ...
    cout<<"build spanning tree.";
    K.build_spt3(ve_delta, edge_info_file);
    cout<<"Done."<<endl;
...
//retrive
    ...
    K.retrieve_1manifold_simp(vpath_file);
